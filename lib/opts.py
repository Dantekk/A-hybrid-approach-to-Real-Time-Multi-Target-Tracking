...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--trainval', action='store_true',
                             help='include validation in training and '
                                  'test on test set')
    self.parser.add_argument('--pretrained', type=int, default=0, help='0=not pretrained, 1=pretrained')
    self.parser.add_argument('--train_only_det', type=int, default=0, help='0=train all, 1=train_only_det')

    ###
    self.parser.add_argument('--dir_base_save',
                             help='dir in cui salvare il tutto')
    ...
    ### Byte mode
    self.parser.add_argument('--byte_mode', type=int, default=0, help='1=Byte_mode')
    ...
    ## Demo
    self.parser.add_argument('--save_frame', default=True)
    self.parser.add_argument('--save_text', default=True)
    self.parser.add_argument('--show_image', default=False)
    self.parser.add_argument('--demo', default=False)

    ## KLT Mode
    self.parser.add_argument('--lkt_num_frame', default=0, help='number of consecutive frame of lkt mode')
  ...
  def parse(self, args=''):
    ...
    opt.save_dir = os.path.join("results/save_model", opt.exp_id)
    opt.debug_dir = os.path.join(opt.save_dir, 'debug')
    print('The output will be saved to ', opt.save_dir)

    if opt.resume and opt.load_model == '':
      model_path = opt.save_dir[:-4] if opt.save_dir.endswith('TEST') \
                  else opt.dir_base_save
